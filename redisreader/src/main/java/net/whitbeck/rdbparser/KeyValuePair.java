...
public final class KeyValuePair implements Entry {
  ...
  public Object asJSON() throws UnsupportedEncodingException {
    ValueType vt=getValueType();
    if (vt==ValueType.VALUE){
      return new String(getValues().get(0), "UTF-8");
    }else if(vt==ValueType.HASH||vt==ValueType.HASHMAP_AS_ZIPLIST){
      JSONObject hash=new JSONObject();
      List<byte[]> raw=getValues();
      for (int i=0;i<=raw.size()-2;i+=2){
        String k=new String(raw.get(i), "UTF-8");
        String v=new String(raw.get(i+1), "UTF-8");
        hash.put(k,v);
      }
      return hash;
    }else{
